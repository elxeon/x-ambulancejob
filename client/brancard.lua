...
RegisterNetEvent("esx_ambulancejob:brancard:spawnstretcher")
AddEventHandler("esx_ambulancejob:brancard:spawnstretcher", function()
    local bedHash = SpawnProp('prop_ld_binbag_01')
    SetEntityHeading(bedHash, GetEntityHeading(PlayerPedId()))
    PlaceObjectOnGroundProperly(bedHash)
end)

RegisterNetEvent("esx_ambulancejob:brancard:deletestretcher")
AddEventHandler("esx_ambulancejob:brancard:deletestretcher", function()
    DeleteOBJ(`prop_ld_binbag_01`)
    exports['qs-core']:Notify("Brancard verwijderd", "success")
    Wait(500)
...
function DeleteOBJ(theobject)
    ...
    if DoesObjectOfTypeExistAtCoords(x, y, z, 2.5, theobject, true) then
        local obj = GetClosestObjectOfType(x, y, z, 2.5, theobject, false, false, false)
        DeleteObject(obj)
    end
    Stretcher = nil
end
...
local vehicleOffsets = {
    [`ambulance1`] = { -0.2, -1.7, -0.7},
    [`ambulance`] = { -0.2, -1.7, -0.7},
    [`ambut6`] = { -0.15, -1.6, -0.4 },
    [`blokkenambu`] = { -0.05, -1.9, -0.6 },
    [`ambusprinteroud`] = { -0.2, -1.6, -0.7 },
    [`ambuamarok`] = { -0.35, -1.9, -0.2 },
}
local bedOffsets = {
    [`prop_ld_binbag_01`] = {
        canPickup = true,
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if not IsPedInAnyVehicle(ped) and DoesEntityExist(closestObject) then
            ...
            if bedData.canPickup and not IsHoldingStretcher and #(pedCoords - pickupCoords) <= 1.2 and (ESX.PlayerData.job and ESX.PlayerData.job.name == 'ambulance' or ESX.PlayerData.job and ESX.PlayerData.job.name == 'police') then
            ...
            if IsHoldingStretcher then
                ...
                if vehicle ~= 0 then
                    ...
                    if distance < 2.0 and vehicleOffsets[model] ~= nil then
                        ...
                        local text = ('[E] In voertuig plaatsen')
...
Sit = function(civiereObject, bedData)
    ...
    if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
        if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'savecouch@', 't_sleep_loop_couch', 3) then
            exports['qs-core']:Notify("iemand gebruikt de brancard al", "error")
            return
        end
    end
...
PickUp = function(civiereObject)
    ...
    if closestPlayer ~= nil and closestPlayerDist <= 1.5 then
        if IsEntityPlayingAnim(GetPlayerPed(closestPlayer), 'anim@heists@prison_heistig1_p1_guard_checks_bus', 'loop', 3) then
            exports['qs-core']:Notify("iemand is bezig met de brancard", "error")
            return
        end
    end
